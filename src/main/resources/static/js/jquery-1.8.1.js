...
(function (d, m) {
    ...
    var i = d.M = {};
    var y = true, af = false, Y = 0, D = 0, F = 1, V = 2, H = String.prototype, p = Array.prototype,
        h = Object.prototype, n = Function.prototype, j = H.trim, O = p.map, ac = p.some, ak = p.every, P = p.slice,
        k = p.filter, u = p.forEach, X = p.indexOf, R = p.lastIndexOf, B = n.bind;
    i.log = function () {
    ...
    var aj = function (ap) {
        ...
        if (!aa(al)) {
            var av = a(al[1]);
            if (av.length > 0) {
                var aq = av.split(","), au = a(aq[0]),
                    aw = new RegExp("\\b" + au + "\\s*\\(\\s*['\"]([^\\)]+)['\"]\\s*\\)", "g");
                var at, an = [];
...
(function () {
    ...
    function d(k) {
        var m = $(k.currentTarget), g = m.closest("[data-cs-t]").data("cs-t"),
            i = m.closest("[data-cs-p]").data("cs-p"), h = (m.attr("target") || "").toLowerCase(), l = m.data("cs-l"),
            e = m.data("cs-d"), f = true;
    ...
    function b(g, h, j, e, f) {
        if (!f) {
            var i = (c.WWW_HOST ? location.protocol + "//" + c.WWW_HOST : "") + "/cs.php";
            $.ajax({url: i, dataType: "jsonp", data: {t: g, p: h, l: j, d: e}, jsonp: "cb"})
        } else {
            if ("/" == j.substr(0, 1) || location.protocol + "//" == j.substr(0, 7).toLowerCase()) {
                var m = c.WWW_HOST ? location.protocol + "//" + c.WWW_HOST : "", k = m + "/cs.php", l = k.length;
                if (k != j.substr(0, l)) {
                    f.attr("href", m + "/cs.php?t=" + encodeURIComponent(g) + "&p=" + encodeURIComponent(h) + "&l=" + encodeURIComponent(j) + "&d=" + encodeURIComponent(e));
                    f.attr("data-href", j)
                }
            } else {
                b(g, h, j, e, null)
    ...
    function a(e) {
        ...
        var f = (c.WWW_HOST ? location.protocol + "//" + c.WWW_HOST : "") + "/ajax/ajax_logs.php";
...
if (!window._m_statistics_js_loaded) {
    window._m_statistics_js_loaded = true;

    function mxPageGuid() {
    }

    mxPageGuid.generate = function () {
        var a = mxPageGuid._getRandomInt, c = mxPageGuid._hexAligner;
    ...
    mfwSendLog.sendByImg = function (g, b, k) {
        var i = new Image(1, 1), j = "mfw_" + Math.floor(2147483648 * Math.random()).toString(36);
        window.mLogImg[j] = i;
    ...
    mfwSendLog.sendByPost = function (f, a, j) {
        var i = window.XMLHttpRequest, b = String(new Date().getTime()) + String(Math.random());
        if (!i) {
            return !1
        }
        var h = new i;
    ...
    var mfwPageEvent = (function () {
        return function (o, j, d, l) {
            ...
            if (o && j) {
                ...
                var e = {sdk_ver: "1.3.5", ac: o, ec: j, u: m, r: a, ex: k, ar: d, t: p, pid: g, rn: h};
                e = MFWSTAT._setChannel(e);
    ...
    var MFWSTAT = {
        isPrepub: false, _getQuery: function () {
            ...
            try {
                ...
                for (var b = 0; b < d.length; b++) {
                    var a = d[b].substring(0, d[b].indexOf("="));
                    var c = d[b].substring(d[b].indexOf("=") + 1);
                    if (!g[a]) {
                        g[a] = c
                    }
                }
            } catch (f) {
                console.warn("銆怱DK銆憅uery error", d)
            }
            return g
        }, _getFlash: function () {
            var d, e;
            ...
            var b = "", d = window.location.hostname, c = d.replace(/\.(com|net|org|cn$)\.?.*/, "");
            if (c.lastIndexOf(".") == -1) {
                b = "." + d
            } else {
            ...
            return h
        }, _isCookiePrepub: function () {
            if (MFWSTAT._getCookie("branpub")) {
                return true
            }
            try {
                ...
                for (var b = 0; b < c.length; ++b) {
                    var a = c[b].split("=");
                    var d = /msrule.[\d\w]+.[\d\w]+.product/;
                    if (d.test(a[0])) {
                        return true
                    }
                }
            } catch (f) {
                console.log("銆怱DK銆慽sCookiePrepub analysis");
                return false
            }
            return false
        }, _getVisitn: function () {
        ...
        }, _channelInit: function () {
            var c = MFWSTAT._getQuery();
            if (c.mfw_chid) {
                MFWSTAT._setCookie("__mfwchid", c.mfw_chid, 24 * 7);
                MFWSTAT._setCookie("__mfwothchid", "")
            } else {
                if (!(MFWSTAT._getCookie("__mfwchid")) && document.referrer) {
                    var a = document.referrer;
                    try {
                        if (a.indexOf(".mafengwo.") == -1 && a.indexOf(".imfw.") == -1) {
                            a = a.split("://")[1];
                            a = a.split("/")[0];
                            MFWSTAT._setCookie("__mfwothchid", "referrer|" + a, 24 * 7)
                        }
                    } catch (b) {
                        console.warn("銆怱DK銆憆eferrer error", a)
                    }
                }
            }
        }, _setChannel: function (d) {
            var c = MFWSTAT._getCookie("__mfwchid");
            var a = MFWSTAT._getCookie("__mfwothchid");
            var b = MFWSTAT._getQuery();
            if (b.mfw_chid) {
                d.mfw_chid = b.mfw_chid
            } else {
                if (c) {
                    d.mfw_chid = c
                } else {
            ...
            var p = document, k = window.location, g = parseInt((+new Date()) / 1000, 10);
            var w = "1.3.5";
            ...
            q = MFWSTAT._setChannel(q);
            return q
        }
    };
    if (!window.Env.statistics_loaded) {
        MFWSTAT.isPrepub = MFWSTAT._isCookiePrepub();
        MFWSTAT._channelInit();
        MFWSTAT._run();
    ...
    var mfwCheckLogData = (function () {
        return function () {
            ...
            var a = {t: c, u: e(b), r: e(d), pid: window.Env.uPageId};
            return xstringify(a)
        }
    })();
    (function () {
        ...
        var a = {
            tmo: 0, init: function (e) {
                ...
                if (MFWSTAT.isPrepub) {
                    a.addEvent(window, "hashchange", a.checkUrl);
                    a.historyInit();
                    a.addEvent(window, "pushState", a.checkUrl);
                    a.addEvent(window, "replaceState", a.checkUrl);
                    a.addEvent(window, "popState", a.checkUrl)
                }
                a.guid = a.createGuid()
            }, checkUrl: function () {
                if (!MFWSTAT.isPrepub) {
                ...
                clearTimeout(a.tmo);
                a.tmo = setTimeout(function () {
                    var g = document.querySelector('meta[property="mfw:pagename"]');
                    var e = "";
                    if (g) {
                        e = g.getAttribute("content")
                    }
                    var f = {pageName: e ? e : "", url: window.location.href};
                    var h = new window.XMLHttpRequest;
                    h.open("POST", "https://mdp.mafengwo.cn/api/pageresources/release/url/match", !0);
                    h.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            var i = JSON.parse(this.response);
                            if (i.code != 0 && i.code != -1) {
                                alert(i.msg)
                            }
                        }
                    };
                    h.send(mfwSendLog.postDataFormat(f))
                }, 500)
                ...
                var g = xstringify, i = encodeURIComponent, h = (new Date()).getTimezoneOffset() / 60,
                    k = c.closing - c.opening, j = k - c.blurTime, e = e || {};
                e = {
                    scrolls: c.scrolls,
                    clicks: c.clicks,
                    opening: c.opening,
...
(function () {
    setTimeout(function () {
        var m = document.getElementsByTagName("script"), l = Array.prototype.slice.call(m, 0), j = "mafengwo.net",
            d = "mfwdev.com", b = "mafengwo.cn", e = 0, g = l.length, a, h, c, k;
        var f = {
            "c.cnzz.com": 1,
            "w.cnzz.com": 1,
            "res.wx.qq.com": 1,
            ...
            "w.mafengwo.cn": 1,
            "hm.baidu.com": 1,
            "static.geetest.com": 1,
            "static.fengkongcloud.com": 1,
            "restapi.amap.com": 1
        };
        for (; e < g; e++) {
            a = l[e].getAttribute("src");
            if (a) {
                c = /^[^\/]*\/\/([^\/]+)\//.exec(a);
                k = c && c[1] in f;
                h = c && ((c[1].indexOf(j) != -1 && c[1].indexOf(j) == c[1].length - j.length) || (c[1].indexOf(d) != -1 && c[1].indexOf(d) == c[1].length - d.length) || (c[1].indexOf(b) != -1 && c[1].indexOf(b) == c[1].length - b.length));
                if (!h && !k) {
                    !!mfwPageEvent && mfwPageEvent("default", "cross_domain_script_record", {
                        src: a,
                        host: c ? c[1] : "empty",
...
(function () {
    var c = {reportImage: "https://topic.mafengwo.cn/ad/c.php?", posList: []};
    var m = 0;
    var d = 0;
    var i = "acm";
    ...
    function w() {
        ...
        for (var y = 0; y < z.length; y++) {
            ...
            var A = v(z[y], "type");
            if (("" != x) && (!n(x))) {
                c.posList.push({id: x, type: A, beingExposed: false, exposedTimes: 0, dom: z[y]})
            }
        }
